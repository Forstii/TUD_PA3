...
def process_time_data(data: NDArray) -> NDArray:
    ...
    return (data - first_timestamp) * 1e-3
...
def filter_data(data: NDArray, window_size: int) -> NDArray:
    """Filter data using a simple moving average approach.
...
def calc_heater_heat_flux(p_heater: float, eta_heater: float) -> float:
    ...
    Returns:
    float: The heat flux in Watt generated by the heater.
    """
...
def calc_convective_heat_flow(k_tank: float,
    ...
    Args:
        ...
        area_tank (float): Surface area of the tank in meters squared.
        t_total (NDArray): Array of total temperatures inside the tank in Celsius.
        t_env (float): Environmental temperature outside the tank in Celsius.
    Returns:
...
def calc_mass(level_data: NDArray,
    ...
    """
    volume_array = level_data * tank_footprint / 1000 # /1000 to convert level mm to meters
    mass_array = volume_array * density
    return mass_array
...
def plot_data(data: pd.DataFrame, metadata: dict[str, str]) -> Figure:
    ...
    Args:
        data (pd.DataFrame): A pandas DataFrame containing the data to be plotted. 
                                The first column is assumed to be the x-axis (time in 
                                seconds) and 
    ...
    plt.gcf().text(0.1, 0.1, metadata["text"], fontsize=10, va='center', ha='left')
